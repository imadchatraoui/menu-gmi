...
const TextLoop = ({
  ...
  const [metrics, setMetrics] = useState({ length: 0, unitWidth: 0, reps: 5 });
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      // Generous multiplier so we never run out of text on the path
      const reps = Math.ceil(length / unitWidth) + 5;
      setMetrics(prev => (prev.length === length && prev.unitWidth === unitWidth && prev.reps === reps ? prev : { length, unitWidth, reps }));
    };

    measure();
    
    // Attempt measurement again once fonts are loaded
    if (typeof document !== 'undefined' && document.fonts?.ready) {
      document.fonts.ready.then(measure).catch(() => {});
    }
    // Also measure after a slight delay to catch late layout shifts on mobile
  ...
  useEffect(() => {
    ...
    const tween = gsap.to(state, {
      offset: direction === 'reverse' ? unitWidth : -unitWidth,
  ...
  return (
    <div ref={rootRef} className={`text-loop ${className}`.trim()} style={style}>
      ...
      >
        ...
        {/* Removed textLength and lengthAdjust to prevent text squishing/overlapping */}
        <text
